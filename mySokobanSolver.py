...
class SokobanPuzzle(search.Problem):
    ...
    def __init__(self, warehouse):
        ...
        self.x_max = max([x for x, y in self.maze.walls])
        self.x_min = min([x for x, y in self.maze.walls])

        self.y_max = max([y for x, y in self.maze.walls])
        self.y_min = min([y for x, y in self.maze.walls])

        self.warehouse = maze
        self.goal = warehouse.goal 
        self.initial warehouse.start 
    ...
    def actions(self, state):
        ...
        x, y = self.state
        if (x-1, y) not in self.walls and (x-1, y) not in self.boxes:
            actions.push('Left')
        if (x+1, y) not in self.walls and (x+1, y) not in self.boxes:
            actions.push('Right')
        if (x, y-1) not in self.walls and (x, y-1) not in self.boxes:
            actions.push('Down')
        if (x, y+1) not in self.walls and (x, y+1) not in self.boxes:
            actions.push('Up')
